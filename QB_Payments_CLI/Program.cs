...
namespace QB_Payments_ConsoleApp
{
    class Program
    {
        ...
        {
            string customerName = "asd";
            string memo = "456"; // Replace with the actual memo

            // Step 1: Query the specific invoice by customer name and memo
            Invoice invoiceToPay = InvoiceReader.QueryInvoiceByCustomerNameAndMemo(customerName, memo);
            ...
            {
                Console.WriteLine($"No active invoice found for customer {customerName} with memo {memo}.");
            }
        }
    }
}
